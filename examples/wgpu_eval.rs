...
fn run() -> Result<(), String> {
    ...
    let test_path = get_arg(&args, "--test").unwrap_or_else(|| "/home/noah/src/bashrs/training/shellsafetybench/splits/test.jsonl".into());
    let max_entries: usize = get_arg(&args, "--max").and_then(|s| s.parse().ok()).unwrap_or(500);

    eprintln!("=== WGPU Eval (Full Forward): ShellSafetyBench ===\n");
    ...
    for (idx, (input, label)) in entries.iter().enumerate() {
        ...
        let rmsnorm = |buf: &mut [f32], seq: usize, dim: usize| {
            let eps = 1e-5f32;
            for si in 0..seq {
                let rms = (buf[si*dim..(si+1)*dim].iter().map(|x| x*x).sum::<f32>() / dim as f32 + eps).sqrt();
                for hi in 0..dim { buf[si*dim+hi] /= rms; }
            }
        };
        ...
        for layer_idx in 0..n_layers {
            ...
            let (gate_w, up_w, down_w) = model.ffn_cache[layer_idx].as_ref()
            ...
            for j in 0..(s as usize * h) { hidden[j] += ffn_out[j]; }
        }
    ...
    eprintln!("Stretch goal: MCC > 0.75 → {}", if mcc > 0.75 { "PASS" } else { "FAIL" });
    ...
    if !safe_losses.is_empty() && !unsafe_losses.is_empty() {
        let avg_safe = safe_losses.iter().sum::<f32>() / safe_losses.len() as f32;
        let avg_unsafe = unsafe_losses.iter().sum::<f32>() / unsafe_losses.len() as f32;
...
fn compute_best_mcc(scores: &[(f32, i32)]) -> (f32, f32, usize, usize, usize, usize) {
    ...
    for &thresh in &thresholds {
        ...
        for &(score, label) in scores {
            ...
            match (pred, label) { (1,1)=>tp+=1, (1,0)=>fp+=1, (0,0)=>tn+=1, (0,1)=>fn_+=1, _=>{} }
        }
        ...
        if mcc > best_mcc { best_mcc = mcc; best = (thresh, tp, fp, tn, fn_); }
    }
