...
fn main() {
    #[cfg(not(feature = "gpu"))]
    { eprintln!("Error: --features gpu required"); std::process::exit(1); }
    #[cfg(feature = "gpu")]
...
fn run() -> Result<(), String> {
    ...
    // Test 1: GPU matmul vs CPU reference
    {
        ...
        for i in 0..m { for j in 0..n { let mut s = 0.0f32;
            for p in 0..k { s += a[i*k+p] * b[p*n+j]; } c_cpu[i*n+j] = s;
        }}
        // GPU
        ...
        let ok = cos > 0.9999;
        eprintln!("  [{}] Matmul {}x{}x{}: cos={cos:.6}, max_diff={maxd:.2e}", if ok {"PASS"} else {"FAIL"}, m, k, n);
        if ok { pass += 1; } else { fail += 1; }
    }

    // Test 2: GPU gemm_backward_a (A @ B^T) vs CPU
    {
        let m = 8u32; let n = 16u32; let k = 32u32;
        let a: Vec<f32> = (0..(m*k) as usize).map(|i| ((i * 11 + 5) % 100) as f32 / 100.0 - 0.5).collect();
        let b: Vec<f32> = (0..(n*k) as usize).map(|i| ((i * 17 + 3) % 100) as f32 / 100.0 - 0.5).collect();
        let mut c_cpu = vec![0.0f32; (m*n) as usize];
        for i in 0..m as usize { for j in 0..n as usize { let mut s = 0.0f32;
            for p in 0..k as usize { s += a[i*k as usize+p] * b[j*k as usize+p]; } c_cpu[i*n as usize+j] = s;
        }}
        ...
        let maxd = max_diff(&c_cpu, &c_gpu);
        let ok = cos > 0.9999;
        eprintln!("  [{}] GEMM_BWD_A {}x{}x{}: cos={cos:.6}, max_diff={maxd:.2e}", if ok {"PASS"} else {"FAIL"}, m, n, k);
        if ok { pass += 1; } else { fail += 1; }
    }
    ...
    // Test 3: NF4 quantize → dequant round-trip
    {
        ...
        let nf4_lut: [f32; 16] = [-1.0, -0.696, -0.525, -0.395, -0.284, -0.185, -0.091, 0.0,
        ...
        let ok = all_zero;
        eprintln!("  [{}] NF4 dequant (all-zero): all={all_zero}", if ok {"PASS"} else {"FAIL"});
        if ok { pass += 1; } else { fail += 1; }
    }

    // Test 4: LoRA forward parity (CPU vs our implementation)
    {
        let s = 4; let h = 16; let out = 32; let r = 4;
        ...
        let alpha = 8.0f32; let scaling = alpha / r as f32;
        // CPU reference: y = x @ w + scaling * x @ A^T @ B^T
        let mut y_cpu = vec![0.0f32; s * out];
        for si in 0..s { for oi in 0..out { let mut sum = 0.0f32;
            for hi in 0..h { sum += x[si*h+hi] * w[hi*out+oi]; }
            // LoRA: h_a = x @ A^T, then h_a @ B^T
            let mut lora_sum = 0.0f32;
            for ri in 0..r {
                let mut ha = 0.0f32;
                for hi in 0..h { ha += x[si*h+hi] * a[ri*h+hi]; }
    ...
    // Test 5: AdamW CPU vs GPU parity
    {
        ...
        let mut m_cpu = vec![0.0f32; n]; let mut v_cpu = vec![0.0f32; n];
        let mut m_gpu = vec![0.0f32; n]; let mut v_gpu = vec![0.0f32; n];
        let (lr, b1, b2, eps, wd) = (1e-3, 0.9, 0.999, 1e-8, 0.01);
        // CPU AdamW
        let bc1 = 1.0 / (1.0 - b1); let bc2 = 1.0 / (1.0 - b2);
        for i in 0..n {
            m_cpu[i] = b1 * m_cpu[i] + (1.0 - b1) * grad[i];
            v_cpu[i] = b2 * v_cpu[i] + (1.0 - b2) * grad[i] * grad[i];
            params_cpu[i] -= lr * (m_cpu[i] * bc1 / (( v_cpu[i] * bc2).sqrt() + eps) + wd * params_cpu[i]);
        }
        // GPU AdamW
        device.adamw_step(&mut params_gpu, &grad, &mut m_gpu, &mut v_gpu, lr, b1, b2, eps, wd, 1)?;
        let cos = cosine_sim(&params_cpu, &params_gpu);
        let maxd = max_diff(&params_cpu, &params_gpu);
        let ok = cos > 0.9999;
        eprintln!("  [{}] AdamW parity: cos={cos:.6}, max_diff={maxd:.2e}", if ok {"PASS"} else {"FAIL"});
        if ok { pass += 1; } else { fail += 1; }
    }

    eprintln!("\n=== Canary Results: {pass} PASS, {fail} FAIL ===");
    if fail > 0 { return Err(format!("{fail} canary tests failed")); }
    Ok(())
}
