...
impl ClassifyTrainer {
    ...
    fn validate(&mut self) -> (f32, f32) {
        ...
        let total = self.val_token_cache.len();

        let val_start = std::time::Instant::now();

        // KAIZEN-013: Use pre-tokenized cache instead of re-tokenizing
        for (i, (ids, label)) in self.val_token_cache.iter().enumerate() {
            let (loss, predicted) = self.pipeline.forward_only(ids, *label);
            total_loss += loss;
            if predicted == *label {
                correct += 1;
            }
