...
mod tests {
    ...
    // Additional command parsing tests
    ...
    // Additional artifact type tests

    #[test]
    fn test_artifact_type_from_str_notebook() {
        assert_eq!(
    ...
    #[test]
    fn test_license_from_str_extended() {
        assert_eq!(
            "cc-by-sa-4.0".parse::<LicenseArg>().unwrap(),
            LicenseArg::CcBySa4
        );
        // gpl3 works (no dashes/dots after normalization)
        assert_eq!("gpl3".parse::<LicenseArg>().unwrap(), LicenseArg::Gpl3);
        assert_eq!("gplv3".parse::<LicenseArg>().unwrap(), LicenseArg::Gpl3);
        // bsd3 and bsd3clause work
        assert_eq!("bsd3".parse::<LicenseArg>().unwrap(), LicenseArg::Bsd3);
        assert_eq!(
            "bsd3clause".parse::<LicenseArg>().unwrap(),
            LicenseArg::Bsd3
        );
    }

    // Export format additional tests

    #[test]
    fn test_export_format_from_str_jupyter() {
        assert_eq!(
