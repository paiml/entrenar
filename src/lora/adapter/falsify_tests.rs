...
fn make_layer_with_weights(d_out: usize, d_in: usize, rank: usize, alpha: f32) -> LoRALayer {
    ...
    *layer.lora_a_mut().data_mut() = crate::sovereign_array::Array1::from_vec(a_data);
    *layer.lora_b_mut().data_mut() = crate::sovereign_array::Array1::from_vec(b_data);
...
fn test_falsify_f_ckpt_005_resume_continues_training() {
    ...
    for step in 0..5 {
        ...
        *layer.lora_b_mut().data_mut() = crate::sovereign_array::Array1::from_vec(updated);
    }
    ...
    for step in 5..10 {
        ...
        let updated: Vec<f32> = b_data.iter().zip(&grad).map(|(w, g)| w - g).collect();
        *resumed_layer.lora_b_mut().data_mut() = crate::sovereign_array::Array1::from_vec(updated);
    }
...
fn test_falsify_f_ckpt_005_resumed_weights_not_reinitialized() {
    ...
    *layer.lora_b_mut().data_mut() = crate::sovereign_array::Array1::from_vec(trained_b.clone());
